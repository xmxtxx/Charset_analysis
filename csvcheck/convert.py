...
def _normalize_enc_for_target(source: str, target: str) -> str:
    ...
    # Treat ASCII as already OK for UTF-8 target
    if t == "utf-8" and s == "ascii":
        return t

    # (Optional) treat UTF-8-SIG as UTF-8 (skip if you want to remove BOM)
...
def convert_folder_files(
    ...
    """Convert all files listed in a folder_result to target encoding. Tracks by source encoding."""
    stats = {
    ...
    for i, file_info in enumerate(folder_result["files"], 1):
        ...
        if dry_run:
            ...
            # You can optionally short-circuit here too (avoids I/O):
            if norm_source == target_encoding.lower():
                stats["already_target"] += 1
                if show_individual and show_progress:
                    print(f"  {Colors.DIM}[SKIP]{Colors.NC} {file_path.name} - already {target_encoding}")
...
def display_conversion_summary(all_conversion_stats: List[Tuple[str, Dict]], target_encoding: str, dry_run: bool = False) -> None:
    ...
    print((f"  Would convert: {Colors.GREEN}{total_converted}{Colors.NC} files") if dry_run else (f"  Successfully converted: {Colors.GREEN}{total_converted}{Colors.NC} files"))
    if total_already > 0:
